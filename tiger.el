...
;; Incorporates enhancements by Jerome Bana <bana_j@epita.fr>.
...
(defgroup tiger nil
  ...
  :prefix "tiger-"
  :group 'languages)
;; (\\w\\|_\\)+ match a keyword with _
(defconst tiger-font-lock-keywords
  `(
    ;; Tiger keywords.
    (,(concat "\\<\\(if\\|then\\|else\\|let\\|in\\|array\\|of\\|type\\|"
              "nil\\|while\\|do\\|to\\|for\\|break\\|end\\|var\\|function\\)\\>")
    ...
    ("\\<function\\s-+\\([a-zA-Z]\\(\\w\\|_\\)*\\)\\s-*(.*)\\(\\s-*:\\s-*\\([a-zA-Z]\\(\\w\\|_\\)*\\)\\)?\\s-*="
     (1 font-lock-function-name-face)
     (4 font-lock-type-face))

   ;; "var : val" type stuff (formal parameters, record fields)
    ("\\<\\([a-zA-Z]\\(\\w\\|_\\)*\\)\\s-*:\\s-*\\([a-zA-Z]\\(\\w\\|_\\)*\\)*\\>"
     (1 font-lock-variable-name-face)
    ...
    (,(concat "\\<\\(print\\|flush\\|getchar\\|ord\\|chr\\|size\\|"
              "substring\\|concat\\|not\\|exi\\|print_intt\\)\\>")
...
(defvar tiger-mode-map nil
  "Keymap for `tiger-mode'.")
...
\\{tiger-mode-map}"
  ...
  ;; nested works only with emacs-21
